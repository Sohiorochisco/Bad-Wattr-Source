...
#define WATTR_INTERRUPT_LINES(ENTRYA,ENTRYB,ENTRYC,ENTRYD)\
	ENTRYC(PIO_PER_P9,0,0,PIO_FELLSR_P9,PIO_MDER_P9,&ade_irq_handler)\
	ENTRYC(PIO_PER_P8,0,0,PIO_FELLSR_P8,PIO_MDER_P8,&ade_zx_handler)\
	...
	ENTRYD(PIO_PER_P28,0,0,PIO_FELLSR_P28,0,&relay_toggle)\
	0/*prevent trailing operators */


//Define as follows:
// X(pio id) 
#define WATTR_OUTPUT_LINES(ENTRYA,ENTRYB,ENTRYC,ENTRYD,ENTRYE)\
	ENTRYD(PIO_PER_P20)\
	ENTRYD(PIO_PER_P21)\
	ENTRYD(PIO_PER_P22)\
	ENTRYD(PIO_PER_P23)\
...
#define WATTR_PERIPH_LINES(ENTRYA,ENTRYB,ENTRYC,ENTRYD)\
	...
	ENTRYA(PIO_PDR_P31,0,0)\
	ENTRYA(PIO_PDR_P30,PIO_PDR_P30,0)\
...
static void wattr_output_config(void)
{
	...
	PIOA->PIO_PER = WATTR_OUTPUT_LINES(X11,X01,X01,X01,X01);
	PIOB->PIO_PER = WATTR_OUTPUT_LINES(X01,X11,X01,X01,X01);
	PIOC->PIO_PER = WATTR_OUTPUT_LINES(X01,X01,X11,X01,X01);
	PIOD->PIO_PER = WATTR_OUTPUT_LINES(X01,X01,X01,X11,X01);
	PIOE->PIO_PER = WATTR_OUTPUT_LINES(X01,X01,X01,X01,X11);
	
	//Enable outputs on the lines
	PIOA->PIO_OER = WATTR_OUTPUT_LINES(X11,X01,X01,X01,X01);
	PIOB->PIO_OER = WATTR_OUTPUT_LINES(X01,X11,X01,X01,X01);
	PIOC->PIO_OER = WATTR_OUTPUT_LINES(X01,X01,X11,X01,X01);
	PIOD->PIO_OER = WATTR_OUTPUT_LINES(X01,X01,X01,X11,X01);
	PIOE->PIO_OER = WATTR_OUTPUT_LINES(X01,X01,X01,X01,X11);
	
	PIOA->PIO_OWER = WATTR_OUTPUT_LINES(X11,X01,X01,X01,X01);
	PIOB->PIO_OWER = WATTR_OUTPUT_LINES(X01,X11,X01,X01,X01);
	PIOC->PIO_OWER = WATTR_OUTPUT_LINES(X01,X01,X11,X01,X01);
	PIOD->PIO_OWER = WATTR_OUTPUT_LINES(X01,X01,X01,X11,X01);
	PIOE->PIO_OWER = WATTR_OUTPUT_LINES(X01,X01,X01,X01,X11);
}
...
void pio_config(void)
{
	...
	if(pioa_icount){
		...
		NVIC_SetPriority(PIOA_IRQn,12);
		NVIC_EnableIRQ(PIOA_IRQn);
	}
	if(piob_icount){
		NVIC_ClearPendingIRQ(PIOB_IRQn);
		NVIC_SetPriority(PIOB_IRQn,12);
		NVIC_EnableIRQ(PIOB_IRQn);
	}
	if(pioc_icount){
		NVIC_ClearPendingIRQ(PIOC_IRQn);
		NVIC_SetPriority(PIOC_IRQn,12);
		NVIC_EnableIRQ(PIOC_IRQn);
	}
	if(piod_icount){
		NVIC_ClearPendingIRQ(PIOD_IRQn);
		NVIC_SetPriority(PIOD_IRQn,12);
